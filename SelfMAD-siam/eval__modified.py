#!/usr/bin/env python3
"""
Evaluation script for SelfMAD-siam models.
...
def main(eval_config):
    ...
    if model_dataset == "unknown" and "_siam_" in model_state_path:
        try:
    ...
    model_state = load_model_with_retry(model_state_path, "siam", model_dataset)
    ...
    if model_state is None:
        ...
        alt_model_path = find_model_file(os.path.dirname(os.path.dirname(model_state_path)), "siam", model_dataset)
        if alt_model_path:
            print(f"Found alternative model file: {alt_model_path}")
            model_state = load_model_with_retry(alt_model_path, "siam", model_dataset)
            if model_state:
    ...
    image_size = 224 if model_type == "vit_mae_large" else (384 if "hrnet" in model_type else 380)
    batch_size = 32
    ...
    # Use the evaluation module for original datasets
    # Pass APCER thresholds to the evaluation function
    original_results, original_targets_outputs_dict, original_class_separated_dict = evaluate(
        model=model,
        test_loaders=test_loaders,
        device=device,
        output_dir=os.path.join(eval_dir, "original"),
        model_name=f"{model_name}_original",
    ...
    # Use the visualization module to generate plots for original datasets
    visualize(
        results=original_results,
        targets_outputs_dict=original_targets_outputs_dict,
        class_separated_dict=original_class_separated_dict,
        output_dir=os.path.join(eval_dir, "original"),
        model_name=f"{model_name}_original",
        model_dataset=model_dataset,
        model_type=model_type,
    ...
    print(f"Original dataset evaluation results saved to {os.path.join(eval_dir, 'original')}")
    ...
    if any(key in eval_config for key in ["LMA_path", "LMA_UBO_path", "MIPGAN_I_path", "MIPGAN_II_path", "MorDiff_path", "StyleGAN_path"]):
        ...
        if custom_test_datasets:  # Only proceed if we have datasets
            ...
            print(f"Custom dataset evaluation results saved to {os.path.join(eval_dir, 'custom')}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('-m', '--model_type', type=str, required=True, help="Model type (e.g., vit_mae_large)")
